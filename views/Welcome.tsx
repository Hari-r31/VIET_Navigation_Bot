...
const Welcome: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen bg-slate-50 flex flex-col overflow-hidden font-sans selection:bg-blue-100">
      ...
      <div className="relative z-10 bg-white/80 backdrop-blur-md border-b border-white/20 shadow-sm flex-shrink-0 py-3 px-6 md:px-8">
      ...
      <div className="flex-1 p-4 md:p-6 min-h-0 overflow-hidden flex flex-col items-center justify-center relative z-10">
        
        {/* Page Title Banner */}
        <div className="flex-shrink-0 text-center py-1 relative mb-4">
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-slate-900 tracking-tight mb-1">
            {language === 'en' ? (
        ...
        <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0">
            ...
            <div className="lg:col-span-7 flex flex-col gap-4 h-full min-h-0">
                ...
                <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 p-5 flex-1 min-h-0 flex flex-col relative overflow-hidden group justify-between">
                    ...
                    <div className="relative z-10 flex-shrink-0 mb-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-2">
                ...
                {/* Bottom Info Row */}
                <div className="grid grid-cols-2 gap-4 h-28 flex-shrink-0">
                    <InfoCard 
                        icon={<School size={16} />}
            ...
            <div className="lg:col-span-5 flex flex-col gap-4 h-full min-h-0">
                ...
                {/* Quick Links */}
                <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 p-5 flex-shrink-0 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50 rounded-full -mr-10 -mt-10 pointer-events-none" />
                    <h3 className="text-base font-bold text-slate-900 mb-3 relative z-10">Quick Links</h3>
                    <div className="grid grid-cols-3 gap-2 relative z-10">
                        <QuickLinkButton icon={<Home size={18} />} label="Home" onClick={() => handleQuickLink('/home')} />
                        <QuickLinkButton icon={<Map size={18} />} label="Navigation" onClick={() => handleQuickLink('/directions')} />
                        <QuickLinkButton icon={<FileText size={18} />} label="Details" onClick={() => handleQuickLink('/fees')} />
                    </div>
                </div>

                {/* Submitted By */}
                <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 p-5 flex-1 min-h-0 flex flex-col overflow-hidden relative">
                    <div className="flex-shrink-0 flex items-center gap-2 text-slate-400 font-bold uppercase tracking-wider text-[10px] mb-3">
                        <Users size={12} /> Submitted By
                    </div>
                ...
                {/* Guidance */}
                <div className="bg-slate-900 text-white rounded-[2rem] p-5 shadow-xl relative overflow-hidden flex-shrink-0 group">
                    {/* Abstract Shapes */}
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center gap-2 text-blue-400 font-bold uppercase tracking-wider text-[10px] mb-2">
                            <GraduationCap size={12} /> Under the Guidance of
                        </div>
                        <h3 className="text-lg font-bold mb-1">Mr. M. Hemanth Kumar</h3>
...
const LanguageButton: React.FC<{ lang: string; subtext: string; onClick: () => void; color: 'blue' | 'emerald' | 'orange' }> = ({ lang, subtext, onClick, color }) => {
    ...
    return (
        <button onClick={onClick} className={`w-full group bg-white border border-slate-100 rounded-2xl p-3 flex items-center justify-between transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-0.5 ${colorClasses[color]}`}>
            <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-slate-50 group-hover:bg-white transition-colors ${iconColors[color]}`}>
                    <span className="text-base font-bold">{lang[0]}</span>
                </div>
                <div className="text-left">
                    <span className="block text-base font-bold text-slate-800 group-hover:text-current transition-colors">{lang}</span>
                    <span className="text-[10px] text-slate-400 font-medium">{subtext}</span>
                </div>
            </div>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center bg-slate-50 group-hover:bg-white transition-colors ${iconColors[color]}`}>
...
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; mainText: string; subText: string; color: 'indigo' | 'violet' }> = ({ icon, title, mainText, subText, color }) => {
    ...
    return (
        <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 p-4 flex flex-col justify-center hover:border-slate-200 transition-all duration-300 hover:-translate-y-1 group">
            <div className={`flex items-center gap-2 font-bold text-[10px] uppercase tracking-wider mb-2 text-slate-400 ${colorClasses[color]} transition-colors`}>
                {icon} {title}
            </div>
...
const QuickLinkButton: React.FC<{ icon: React.ReactNode; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
    <button onClick={onClick} className="flex flex-col items-center justify-center gap-2 bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 border border-slate-100 hover:border-blue-100 rounded-2xl p-3 transition-all duration-200 active:scale-95">
...
const TeamMember: React.FC<{ name: string; id: string }> = ({ name, id }) => (
  <div className="flex items-center justify-between p-2 bg-slate-50/50 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-100 transition-colors group">
    <div className="flex items-center gap-2">
        <div className="w-6 h-6 rounded-full bg-white border border-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-400 group-hover:text-blue-500 group-hover:border-blue-100 transition-colors">
            {name.charAt(0)}
        </div>
        <span className="font-bold text-slate-700 text-[10px] md:text-xs group-hover:text-slate-900">{name}</span>
    </div>
    <span className="font-mono text-slate-400 text-[10px] bg-white px-1.5 py-0.5 rounded-md border border-slate-100 group-hover:border-slate-200">{id}</span>
  </div>
);
